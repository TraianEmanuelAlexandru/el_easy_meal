...
import 'package:http/http.dart' as http;
 ...
 /* List<Recipe> listRecipe = [];
  ...
  Future<List<Recipe>> getRecipes() async {
    var result = await http.get(Uri.parse(" https://api.spoonacular.com/recipes/random?number=30&instructionsRequired=true&apiKey=c957b6816ba048139fbc25a67d2cff33"));
    var toJson = jsonDecode(result.body);
    for (int i = 0; i < toJson.length; i++){
      var id = toJson[i]["id"];
      var type = RecipeType.food;
      var name = toJson[i]["name"];
      var duration = toJson[i]["readyInMinutes"];
      List<String> ingredients = toJson[i]["extendedIngredients"];
      var imgUrl = toJson[i]["image"];
